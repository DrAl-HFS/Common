...
U8 ads1xGetRate (const U8 cfg[2]) { return((cfg[1] >> ADS1X_SH1_DR) & ADS1X_DR_M); }
...
void ads10SetRate (U8 cfg[2], const enum ADS10Rate rate) { cfg[1]= (cfg[1] & ~(ADS1X_DR_M << ADS1X_SH1_DR)) | (rate << ADS1X_SH1_DR); }
void ads11SetRate (U8 cfg[2], const enum ADS11Rate rate) { cfg[1]= (cfg[1] & ~(ADS1X_DR_M << ADS1X_SH1_DR)) | (rate << ADS1X_SH1_DR); }
...
F32 ads1xGainToFSV (const enum ADS1xGain g)
{
   ...
   if (g < ADS1X_GAIN_0V256) return(gfsv[g]); else return(0.256);
} // ads1xGainToFSV
